...
**Delivery status:** implemented and verified locally and against a separate cloud Supabase demo with sixteen fictional students. Vercel and Supabase CLI access work. The cloud demo database is active; the hosted frontend preview and production are not live. The current Vercel Hobby plan's eligibility is unresolved; a [Netlify Free deployment option](NETLIFY.md) is prepared and needs Netlify account authorization. See [TEST-AND-DEPLOY.md](TEST-AND-DEPLOY.md) for the remaining setup. No real student information should be entered until deployed authorization, physical-device handoffs, and live backups have been verified.
...
| Deployment | Separate hosting project, same Git repository, app root `projects/art-class-checkin`. Vercel is the preferred option if eligible; a separate Netlify Free fictional preview is prepared. The main site's Vercel configuration stays intact. |
...
The PWA has no Apple membership fee and needs no Mac. The active Supabase demo adds $0/month in subscription charges. Hosting costs depend on the connected plan and usage; no hosting purchase has been made. Domain renewal remains the existing owner's expense.

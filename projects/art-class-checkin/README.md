...
**Delivery status:** implemented and runnable against isolated local Supabase with sixteen fictional students. Cloud preview and production are not live: Vercel and Supabase account access require owner sign-in. See [TEST-AND-DEPLOY.md](TEST-AND-DEPLOY.md) for the exact remaining account actions and deployment sequence. No real student information should be entered until cloud persistence, current grants, physical-device handoffs, and backups have been verified.
...
| Demo/preview | A completely different Supabase project with fictional access fixtures, database UUID, secrets and mode. Production variables must never be assigned to Preview. Local Docker is a third, independent test environment. |
...
**Actual account plans, active cloud database, available quotas and existing paid coverage could not be verified while the dashboards were signed out. No subscription was purchased.** The following are published prices, not a claim about this account's bill.

Vercel Hobby is limited to personal, noncommercial use and is not suitable for this art business. Vercel Pro currently has a $20/month platform fee including one deploying seat and a $20 usage credit; extra deploying seats are $20/month, with usage overages possible. A separate project may add no base charge if this team already has suitable paid coverage, but that must be checked in Billing first. [Hobby terms](https://vercel.com/docs/plans/hobby), [Pro plan](https://vercel.com/docs/plans/pro-plan).
...
Published Supabase daily backup retention is seven days on Pro, fourteen on Team, and up to thirty on Enterprise. Database backups do not include Storage file contents. This app additionally supplies encrypted, app-only logical backup/restore tools, but no unattended cloud backup job has been configured. The owner must establish and verify a backup schedule before live use. [Supabase backups](https://supabase.com/docs/guides/platform/backups).
